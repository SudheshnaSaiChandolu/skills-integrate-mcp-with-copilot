...
This FastAPI application uses SQLAlchemy with a default SQLite database
to persist activities and participants. The previous in-memory data is
...
def init_db_and_seed():
    ...
    try:
        ...
        if existing == 0:
            ...
            for name, info in initial_activities.items():
                act = Activity(
                    name=name,
                    description=info.get("description"),
                    schedule=info.get("schedule"),
                    max_participants=info.get("max_participants"),
                )
                db.add(act)
                db.flush()  # get id
